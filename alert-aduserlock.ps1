...
$lockedLog = Get-EventLog security | where instanceid -EQ 4740 | select -First 1
...
$user = Get-Eventlog security | where index -EQ $indexlog | select -First 1| select -ExpandProperty ReplacementStrings | select -First 2
